...
class MinBase(MPBase, Client):
    ...
    def __init__(self, args, worker_id, train_dataset, test_dataset):
        ...
        self.target_sign = np.random.choice([-1, 1], size=model_shape)
        # print_target_sign(self.target_sign)
    ...
    def omniscient(self, clients):
        ...
        attack_vec = Min(clients, self.__class__.__name__,
                         'unit_vec', self.gamma_init, self.stop_threshold, self.target_sign)
...
def Min(clients, type, dev_type, gamma_init, stop_threshold, target_sign):
    ...
    while np.abs(lamda_succ - lamda) > stop_threshold:
        ...
        if dev_type == 'sign_shift':
            mal_update = sign_shift(benign_mean, lamda, target_sign)
        else:
    ...
    if dev_type == 'sign_shift':
        lamda_succ = lamda_succ if lamda_succ != 0 else 1
        mal_update = sign_shift(benign_mean, lamda_succ, target_sign)
    else:
        mal_update = benign_mean - lamda_succ * deviation
    # print(f"Lamda_succ: {lamda_succ}")
    return mal_update

def sign_shift(benign_mean, lamda, target_sign):
    benign_mean_sign = np.sign(benign_mean)
    benign_mean[benign_mean_sign == target_sign] *= lamda
    benign_mean[benign_mean_sign != target_sign] *= -lamda
    # print(f"match: {np.mean(benign_mean_sign == target_sign)}")
    # print(f"No match: {np.mean(benign_mean_sign != target_sign)}")
